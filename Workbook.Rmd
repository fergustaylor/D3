...
Make a limited databse for JSON Format
```{r}
...
faultycharacters <- str_detect(master$name, "\\(") | str_detect(master$name, string="\\)") | grepl(pattern = "/", x = master$name)
...
fixedvector <- lapply(faultyvalues, function(x) 
  x %>%
  stri_replace_all_regex(pattern = "/", replacement = " ") %>%
  str_replace_all(pattern = "\\(", replacement = "") %>%
  str_replace_all(pattern = "\\)", replacement = "")
  )
fixedvector <- unlist(fixedvector)

master$imports <- master$imports %>%
  stri_replace_all_regex(pattern = "/", replacement = " ")

master$imports %>%
  stri_replace_all_regex(pattern = "/", replacement = " ")

faultyvalues2 <- faultyvalues %>%
  str_replace_all(pattern = "\\(", replacement = "\\\\(") %>%
  str_replace_all(pattern = "\\)", replacement = "\\\\)")

for (i in 1:length(faultyvaluesindex)){
  master$imports <- master$imports %>%
    str_replace_all(pattern = faultyvalues2[i], replacement = fixedvector[i])
}
...
#Add totals to the original master
```{r}
master <- master %>%
