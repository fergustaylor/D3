...
////
Will reshuffle this later. But adding totals to the original master
```{r}
master2 <- master %>%
...
master2 <- master2 %>%
  mutate(studytot = str_count(master$Severity, "Study")) %>%
  mutate(anectot = str_count(master$Severity, "Anecdotal")) %>%
  mutate(theorettot = str_count(master$Severity, "Theoretical")) %>%
  mutate(nsetot = str_count(master$Severity, "NotSet"))

master2 %>%
  jsonlite::toJSON() %>%
  write(file="archive/flare2electricboogaloo.json")
```
